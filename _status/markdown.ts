/**
 * Markdown renderer for a RepoStatus snapshot.
 * Shared between server.ts (POST /api/markdown), build.ts and the legacy
 * generate-status-md.ts so the output format stays in one place.
 */

import type { RepoStatus } from "./scanner";

export function renderMarkdown(status: RepoStatus): string {
  const lines: string[] = [];
  ...
  lines.push("");
  lines.push(
    `> Snapshot généré le **${date}** — branche \`${status.branch}\`${
      status.commitsAhead
        ? ` (${status.commitsAhead} commit(s) en avance sur origin/main)`
        : ""
    }`,
  );
  lines.push("");
  ...
  lines.push("|---|---|");
  lines.push(
    `| Lignes de code total | **${status.totals.loc.toLocaleString("fr-FR")}** |`,
  );
  lines.push(
    `| Fichiers source | ${status.totals.files.toLocaleString("fr-FR")} |`,
  );
  ...
  lines.push(
    `| Commits 30 derniers jours | ${status.totals.commitsLast30d} |`,
  );
  lines.push("");
  ...
  for (const p of status.projects) {
    lines.push(
      `| ${p.emoji} | **${p.name}** | ${p.stack} | ${p.loc.toLocaleString("fr-FR")} | ${p.files} | ${p.testCount} |`,
    );
  }
  lines.push("");
  ...
  for (const p of status.projects) {
    ...
    lines.push(
      `- LOC : **${p.loc.toLocaleString("fr-FR")}** dans ${p.files} fichiers`,
    );
  ...
  lines.push("");
  lines.push(
    `Le CLAUDE.md global impose une cible de **200-300 lignes max par fichier**. ${status.bigFiles.length} fichiers dépassent.`,
  );
  lines.push("");
  ...
  if (status.todos.length) {
    ...
    for (const t of status.todos.slice(0, 30)) {
      lines.push(
        `- **${t.kind}** \`${t.file}:${t.line}\` — ${t.text || "_(sans texte)_"}`,
      );
    }
