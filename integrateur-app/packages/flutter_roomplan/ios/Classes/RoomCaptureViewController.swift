...
@objc public class RoomCaptureViewController: UIViewController, RoomCaptureViewDelegate, RoomCaptureSessionDelegate {
    ...
    Task {
        // Export JSON (required for floor plan conversion)
        let jsonSuccess = await exportToJSON()
        // Export USDZ (optional, best-effort)
        let _ = await exportToUSDZ()

        await MainActor.run {
            self.activityIndicator.stopAnimating()

            if jsonSuccess {
                FlutterRoomplanPlugin.sharedChannel?.invokeMethod("onRoomCaptureFinished", arguments: nil)
                print("[FlutterRoomplan] JSON export succeeded, notified Flutter")
            } else {
                print("[FlutterRoomplan] JSON export failed")
            }
